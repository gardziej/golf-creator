define(['system/myHelper', 'system/Canvas', 'system/Vector2', 'system/Mouse', 'app/bars/Bar',
	...
	function ToCartBar (parent, tilesManager)
		{
	...
	ToCartBar.prototype = Object.create(Bar.prototype);

	ToCartBar.prototype.calculatePosition = function (size, tilesManager) {
		var position = new Vector2(
			canvas.width - tilesManager.sumBar.width -size.x - 20,
			canvas.height - size.y - tilesManager.pricingBar.size.y - 20
		);
		return position;
	};

	ToCartBar.prototype.clicked = function () {
		this.app.goToCart = true;
	}

	ToCartBar.prototype.update = function (delta) {
		this.position = this.calculatePosition(this.size, this.tilesManager);
		this.textPosition = new Vector2 (
			(this.position.x + this.width / 2), (this.position.y + this.height / 2)
		);
		this.text = this.app.appDataObj.line('go_to_cart');
	}

	ToCartBar.prototype.draw = function () {
		if (this.visible)
			{
